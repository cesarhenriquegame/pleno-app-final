...
import { 
  Menu, X, Sun, Moon, Type, Volume2, Play, Pause, 
  Facebook, Instagram, Youtube, MapPin, Phone, Globe, Heart, Mail, HandHeart, ArrowRight, Home, ChevronDown, Trophy, Radio, Calendar, Video
} from 'lucide-react';
    ...
    <>
      <nav className="fixed w-full z-40 top-0 font-sans">
        <div className="relative bg-white/95 dark:bg-navy-900/95 backdrop-blur-xl shadow-lg border-b border-slate-200 dark:border-navy-800 transition-all duration-500 flex flex-col items-center">
          ...
          {/* Top Bar: Logo Centralizado + Ações Laterais */}
          <div className="w-full max-w-7xl mx-auto px-4 h-16 md:h-20 flex items-center justify-center relative z-20">
            ...
            <Link to="/" className="transform hover:scale-105 transition-transform duration-300 flex justify-center items-center">
               {/* Logo da Igreja - Caminho direto da public */}
               <img 
                 src="/logo.png" 
                 alt="Logo Igreja Pleno em Cristo" 
                 className="h-8 md:h-12 w-auto object-contain" 
               />
            </Link>

            {/* Ações à direita (Valente + Tema) */}
            <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-3">
               {/* Botão Valente */}
               <button
                  onClick={() => setIsValenteModalOpen(true)}
                  className="hidden sm:flex items-center gap-2 px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-full font-bold text-xs uppercase tracking-wider shadow-lg transition-all hover:scale-105 animate-pulse-slow"
                >
                  <Trophy size={14} />
                  VIRE UM VALENTE
               </button>
               ...
               <button
                  ...
                  className="p-2 rounded-full bg-slate-100 dark:bg-navy-800 text-slate-600 dark:text-slate-300 hover:text-gold-500 transition-colors shadow-sm"
          ...
          {/* Menu Expansível (Desce da barra) - Desktop Only or explicit trigger */}
          <div 
            className={`hidden md:block w-full overflow-hidden transition-all duration-500 ease-in-out bg-slate-50/50 dark:bg-navy-950/50 border-t border-slate-100 dark:border-navy-800
          ...
          >
            <div className="max-w-7xl mx-auto px-4 flex flex-col items-center gap-6">
              ...
              <div className="flex flex-wrap justify-center gap-x-8 gap-y-4">
                {navLinks.map((link) => {
                  const isKids = link.path === '/kids';
                  return (
                    <Link
                      key={link.name}
                      to={link.path}
                      className={`text-base font-medium transition-all duration-300 hover:-translate-y-0.5 relative group
                        ${isKids 
                          ? 'text-transparent bg-clip-text bg-gradient-to-r from-pink-500 via-purple-500 to-blue-500 font-bold hover:opacity-80' 
                          : isActive(link.path) 
                            ? 'text-navy-900 dark:text-white font-bold' 
                            : 'text-slate-600 dark:text-slate-400 hover:text-gold-600 dark:hover:text-gold-400'
                        }`}
                    >
                      {link.name}
              ...
              {/* Botão Ofertar no Menu */}
              <Link
                  to="/ofertar"
                  className="px-8 py-3 bg-gradient-to-r from-gold-500 to-gold-600 text-navy-900 rounded-full font-bold text-sm hover:from-gold-400 hover:to-gold-500 transition-all shadow-lg shadow-gold-500/20 flex items-center gap-2 transform hover:scale-105"
                >
          ...
          {/* Botão Seta (Trigger) - "Pendurado" - Desktop Only */}
          <button
            onClick={() => setIsMenuOpen(!isMenuOpen)}
            className="hidden md:flex absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-[80%] z-30 
                       bg-white dark:bg-navy-900 text-gold-500 
                       border border-slate-200 dark:border-navy-700 
                       w-12 h-12 rounded-full items-center justify-center 
                       shadow-lg hover:scale-110 hover:shadow-xl transition-all duration-300 group"
            aria-label="Abrir Menu"
          >
            <ChevronDown 
              size={24} 
              className={`transition-transform duration-500 ease-out ${isMenuOpen ? 'rotate-180' : 'group-hover:translate-y-0.5'}`} 
            />
          </button>
      ...
      {isValenteModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-navy-900/90 backdrop-blur-sm animate-fade-in">
          <div className="bg-white dark:bg-navy-900 p-8 rounded-3xl shadow-2xl border border-orange-500/30 max-w-md w-full text-center relative animate-slide-up">
            <button 
              onClick={() => setIsValenteModalOpen(false)} 
              className="absolute top-4 right-4 text-slate-400 hover:text-red-500 transition-colors p-1 bg-slate-100 dark:bg-navy-800 rounded-full"
            >
              <X size={20}/>
            </button>
            
            <div className="w-20 h-20 bg-orange-100 dark:bg-orange-900/20 rounded-full flex items-center justify-center mx-auto mb-6 text-orange-500 shadow-inner">
              <Trophy size={40} />
            </div>
            ...
            <p className="text-slate-600 dark:text-slate-300 mb-8 text-lg leading-relaxed italic">
              "Quer ser uma valente? Venha comigo!"
            </p>
            
            <a 
              href="https://valente.plenoemcristo.com.br/#transformacao" 
              target="_blank" 
              rel="noopener noreferrer"
              className="block w-full py-4 bg-gradient-to-r from-orange-500 to-orange-600 text-white font-bold rounded-xl shadow-lg shadow-orange-500/30 hover:scale-105 transition-transform uppercase tracking-wide text-sm"
              onClick={() => setIsValenteModalOpen(false)}
            >
              QUERO PARTICIPAR
            </a>
        ...
        <div className="flex justify-around items-center h-16">
          {navItems.map((item, idx) => (
            <button
              ...
              className={`flex flex-col items-center justify-center w-full h-full relative no-select
                ${item.isMain ? '-mt-6' : ''}`}
            >
              {item.isMain ? (
        ...
        <div className="fixed inset-0 z-[60] md:hidden">
           ...
           <div className="absolute bottom-0 left-0 right-0 bg-white dark:bg-navy-900 rounded-t-3xl p-6 animate-slide-up max-h-[80vh] overflow-y-auto">
              ...
              <div className="flex items-center justify-between mb-6">
                 ...
                 <button onClick={() => setIsMenuSheetOpen(false)} className="p-2 bg-slate-100 dark:bg-navy-800 rounded-full">
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                 {menuLinks.map((link) => (
                   <Link 
                     ...
                     className="flex items-center p-4 bg-slate-50 dark:bg-navy-800 rounded-xl text-navy-900 dark:text-white font-bold text-sm border border-slate-100 dark:border-navy-700 active:bg-gold-100 dark:active:bg-navy-700 transition-colors"
                   >
                     {link.name}
                   </Link>
                 ))}
...
const PersistentPlayer: React.FC = () => {
  ...
  return (
    <div className={`fixed left-0 right-0 bg-navy-950 border-t border-gold-600/30 z-30 px-4 py-3 shadow-[0_-4px_20px_rgba(0,0,0,0.1)] transition-all duration-300
      bottom-[64px] md:bottom-0`}> {/* Adjusted bottom for mobile nav */}
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="hidden md:flex items-end gap-1 h-8">
            {[...Array(5)].map((_, i) => (
              <div 
                key={i} 
                className={`w-1 bg-gold-500 rounded-t transition-all duration-150 ${isPlaying ? 'animate-[pulse_0.5s_ease-in-out_infinite]' : 'h-1'}`}
                style={{ height: isPlaying ? `${Math.random() * 20 + 5}px` : '4px', animationDelay: `${i * 0.1}s` }}
              />
            ))}
          </div>
          ...
          <button 
            onClick={togglePlay}
            className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-full bg-gold-500 text-navy-900 hover:bg-gold-400 transition-transform hover:scale-105 shadow-lg shadow-gold-500/20"
          >
    ...
    <div className="fixed bottom-32 md:bottom-24 right-4 z-30 flex flex-col items-end gap-2">
      {isOpen && (
        <div className="bg-white dark:bg-navy-900 text-navy-900 dark:text-white p-4 rounded-lg shadow-xl mb-2 flex flex-col gap-3 min-w-[200px] animate-slide-up border border-slate-100 dark:border-navy-800">
          ...
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium">Libras</span>
            <span className="text-xs bg-blue-100 text-blue-800 px-2 py-0.5 rounded">Ativo no canto</span>
          </div>
        </div>
      )}
      ...
      <button 
        ...
        className="w-10 h-10 md:w-12 md:h-12 bg-white dark:bg-navy-900 border-2 border-gold-500 rounded-full flex items-center justify-center text-gold-500 shadow-lg hover:bg-slate-50 dark:hover:bg-navy-800 transition-colors"
...
const FloatingHomeButton: React.FC = () => {
  ...
  // Não mostrar na página inicial ou em mobile (já tem na barra inferior)
  ...
  return (
    ...
    >
      <Home size={22} />
      <span className="absolute left-14 bg-navy-900 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
        Voltar ao Início
      </span>
    </Link>
  );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <div className="space-y-6">
            <div>
               ...
               <p className="text-gray-400 text-sm leading-relaxed">
                 Transformando vidas através da palavra de Deus. Um lugar de novos começos, cura e restauração.
               </p>
            </div>
            <div className="flex space-x-4 pt-2">
              <a href="https://www.facebook.com/ieplenoemcristo" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-navy-900 flex items-center justify-center text-gray-400 hover:text-gold-500 hover:bg-navy-800 transition-all border border-navy-800" aria-label="Facebook"><Facebook size={20}/></a>
              <div className="group relative flex items-center justify-center">
                 <a href="#" onClick={(e) => e.preventDefault()} className="w-10 h-10 rounded-full bg-navy-900 flex items-center justify-center text-gray-400 hover:text-gold-500 hover:bg-navy-800 transition-all border border-navy-800 cursor-not-allowed" aria-label="Instagram"><Instagram size={20}/></a>
                 <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-black text-white text-xs py-1 px-2 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">Em breve</span>
              </div>
              <a href="https://www.youtube.com/@ieplenoemcristo" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-navy-900 flex items-center justify-center text-gray-400 hover:text-gold-500 hover:bg-navy-800 transition-all border border-navy-800" aria-label="YouTube"><Youtube size={20}/></a>
            </div>
          </div>
            ...
            <ul className="space-y-6 text-gray-400">
              <li className="group">
                <div className="flex items-center gap-2 text-white font-bold mb-1">
                  <Mail size={16} className="text-gold-500" /> Geral
                </div>
                <p className="text-sm hover:text-gold-400 transition-colors">igrejaplenoemcristo@gmail.com</p>
              </li>
              <li className="group">
                <div className="flex items-center gap-2 text-white font-bold mb-1">
                  <Phone size={16} className="text-gold-500" /> Secretaria
                </div>
                <p className="text-sm hover:text-gold-400 transition-colors">(11) 2679-5714</p>
              </li>
              <li className="group">
                 <div className="flex items-center gap-2 text-white font-bold mb-1">
                   <HandHeart size={16} className="text-gold-500" /> Voluntário
                 </div>
                 <p className="text-xs mb-1">Faça parte e aprenda:</p>
                 <p className="text-sm hover:text-gold-400 transition-colors">igrejaplenoemcristo@gmail.com</p>
              </li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3 text-gray-400">
              <li className="flex justify-between border-b border-white/5 pb-3 hover:pl-2 transition-all">
                <span>Domingo (Celebração)</span>
                <span className="text-gold-500 font-bold">18:00</span>
              </li>
              <li className="flex justify-between border-b border-white/5 pb-3 hover:pl-2 transition-all">
                <span>Quarta (Ensino)</span>
                <span className="text-gold-500 font-bold">19:30</span>
              </li>
              <li className="flex justify-between border-b border-white/5 pb-3 hover:pl-2 transition-all">
                <span>Sexta (Libertação)</span>
                <span className="text-gold-500 font-bold">20:00</span>
              </li>
            </ul>
          </div>
          ...
          <div>
             ...
             <div className="bg-navy-900 p-6 rounded-2xl border border-navy-800 shadow-lg hover:border-gold-500/50 transition-colors">
               <p className="text-gray-300 text-sm mb-6 leading-relaxed">
                 Contato seguro para doações e ofertas? Faça diretamente clicando no botão abaixo:
               </p>
               <Link 
                 to="/ofertar"
                 className="bg-gold-500 text-navy-900 font-bold py-3 px-6 rounded-lg hover:bg-gold-400 transition-all shadow-lg shadow-gold-500/20 w-full text-center flex items-center justify-center gap-2 group"
               >
